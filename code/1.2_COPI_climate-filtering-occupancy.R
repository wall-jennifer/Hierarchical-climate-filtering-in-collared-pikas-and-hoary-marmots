# COPI_climate-filtering-occupancy
...
# Organize the data
COPI.jags_data = list(J = J, K = K, y = data.COPI[, c("Scat", "Indiv", "Vocal")], 
                      Z = ifelse(rowSums(data.COPI[, c("Scat", "Indiv", "Vocal")], na.rm = T) > 0, 1, NA),
                      cloud = scaled.COPI$cloud, avgdiam = scaled.COPI$avgdiam, gramforb = scaled.COPI$gramforb)
...
sink("COPI_simpleoccupancy.jags")
cat("
...
model{
  ...
  b0[1] ~ dnorm(0,0.001)  # scat
  b0[2] ~ dnorm(0,0.001)  # individual sightings
  b0[3] ~ dnorm(0,0.001)  # vocalizations
...
COPI_testocc_sim <- jagsUI::jags(data = COPI.jags_data, 
                                 inits=inits,
                                 model.file = "COPI_simpleoccupancy.jags",
...
mcmcOutput::diagPlot(COPI_testocc_sim) # diagnostic plots
print(COPI_testocc_sim)
